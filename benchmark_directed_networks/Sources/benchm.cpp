...
int build_subgraph(deque<set<int> > & Ein, deque<set<int> > & Eout, const deque<int> & nodes, const deque<int> & d_in, const deque<int> & d_out) {
	...
	for(int i=0; i<self_loop.size(); i++) {
		...
		while (stop++ < stopper) {
			...
			while(true) {
				...
				int random_mate=irand(d_in.size()-1);
	...
	for(int node_a=0; node_a<d_in.size(); node_a++) for(int krm=0; krm<en_out[node_a].size(); krm++) {
				
				
		int random_mate=irand(d_in.size()-1);
		while (random_mate==node_a)
			random_mate=irand(d_in.size()-1);
	...
	for (int i=0; i<multiple_edge.size(); i++) {
		...
		while (true) {
			...
			int random_mate=nodes[irand(d_in.size()-1)];
			while (random_mate==a || random_mate==b)
				random_mate=nodes[irand(d_in.size()-1)];
